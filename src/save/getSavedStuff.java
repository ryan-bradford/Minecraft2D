...
public class getSavedStuff {
	...
	public static ArrayList<ArrayList<block>> getNextScreen() {
		...
		for (int i = chunkStart; i < chunkEnd; i = i + 4) {
			block currentBlock = new block(
					main.getImageFileNames()[Integer.parseInt(text[i + 2]
							.trim())], Integer.parseInt(text[i + 2].trim()));
			currentBlock.health = Integer.parseInt(text[i + 3].trim());
			currentBlock.setBounds(Integer.parseInt(text[i].trim()),
					Integer.parseInt(text[i + 1].trim()), main.blockHeight,
					main.blockHeight);
			screen.get(Integer.parseInt(text[i + 1].trim()) / main.blockHeight)
					.add(currentBlock);
		}
		return screen;
	}
	...
	public static Integer[] getPlayerBounds() {
		...
		if (text != null) {
			...
			return new Integer[] {
					Integer.parseInt(text[currentRow + 1].trim()),
