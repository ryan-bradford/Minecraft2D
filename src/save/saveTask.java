...
public class saveTask extends task {
	...
	public void saveInventory() {
		...
		for (int i = 0; i < main.map.inventory.inventoryButtons.length; i++) {
			for (int x = 0; x < main.map.inventory.inventoryButtons[i].length; x++) {
				out.write(Integer
						.toString(main.map.inventory.inventoryButtons[i][x].amount));
				out.println(" ");
				out.write(Integer
						.toString(main.map.inventory.inventoryButtons[i][x].blockID));
				out.println(" ");
			}
		}
		out.write("End");
	...
	public void saveInventoryBar() {
		...
		for (int i = 0; i < main.map.inventoryBar.inventoryBarButtons.length; i++) {
			out.write(Integer
					.toString(main.map.inventoryBar.inventoryBarButtons[i].amount));
			out.println(" ");
			out.write(Integer
					.toString(main.map.inventoryBar.inventoryBarButtons[i].blockID));
			out.println(" ");
		}
		out.write("End");
		out.println(" ");
