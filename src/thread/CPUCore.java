...
public class CPUCore extends Thread {
	...
	public CPUCore(ArrayList<task> tasks1) {
		load = 0;
	...
	public void run() {
		while (true) {
			try {
				try {
					CPUCore.sleep(shortestWait);
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
				for (int i = 0; i < tasks.size(); i++) {
					if (numShortWaitsPassed.get(i) + 1 * shortestWait >= waits.get(i)) {
						numShortWaitsPassed.set(i, 0);
						if (tasks.get(i).returnRunnable() == true) {
							tasks.get(i).runTask();
						}
					} else {
						numShortWaitsPassed.set(i, numShortWaitsPassed.get(i) + 1);
					}
				}
			} catch (NullPointerException ex) {
			}
